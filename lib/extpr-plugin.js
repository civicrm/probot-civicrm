...
module.exports = (robot) => {
  ...
  commands(robot, 'test', async (context, command) => {
    ...
    const response = await context.github.pulls.get({owner: issue.owner, repo: issue.repo, pull_number: issue.number})
  ...
  async function testRepoRev (context, gitUrl, gitBaseRev, gitHeadRev, sourceUrl) {
    ...
    if (!repoAuthz) {
      robot.log.warn('Ignore unauthorized repo: ' + repo.owner + '/' + repo.repo)
      return
    }
    ...
    if (infoxml === null || infoxml === '') {
      robot.log.warn('Failed to read info.xml: ' + repo.owner + '/' + repo.repo)
      return
    }

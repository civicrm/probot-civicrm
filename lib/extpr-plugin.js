...
module.exports = (robot) => {
  ...
  robot.on(['pull_request.opened', 'pull_request.reopened', 'pull_request.synchronize'], async (context) => {
    ...
    await testRepoRev(context, context.payload.repository.clone_url, pr.pull_request.base.ref, pr.pull_request.head.sha)
  })
  ...
  commands(robot, 'test', async (context, command) => {
    ...
    await testRepoRev(context, pr.base.repo.clone_url, pr.base.ref, pr.head.sha)
  })
   ...
   */
  async function testRepoRev (context, gitUrl, gitBaseRev, gitHeadRev) {
